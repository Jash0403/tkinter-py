from tkinter import *
root = Tk()
root.geometry("287x300")
title=Label(root,text="Welcome to Real Time Currency Converter",width=40, justify=CENTER,fg="white")
title.config(background="dark blue", pady=2, padx=2)
title.grid(row=0, column=0, columnspan=2)
money = Label(root, text="1 USD = 74.93 Indian Rupee\nDate:2022-05-04",font=("bold", 13))
...
def convert():
    ...
    if fromCurr == "USD":
        if toCurr == "USD":
            ...
            moneyEntry2.config(text=str(float(moneyEntry.get()) * 74.93))
        elif toCurr == "EURO":
